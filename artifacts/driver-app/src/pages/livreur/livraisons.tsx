...
export default function LivreurLivraisons() {
  ...
  return (
    <LivreurLayout>
      <div className="flex-1 p-5 md:p-8 space-y-5 animate-in fade-in duration-300 relative min-h-full" style={{ background: "linear-gradient(135deg, #1A0A06 0%, #2C1810 100%)" }}>
        ...
        {/* Filters */}
        <div
          className="flex flex-col md:flex-row gap-3 items-center p-4 rounded-2xl border relative z-10"
          style={GLASS_STYLE}
        >
        ...
        <div className="space-y-3 relative z-10">
          {isLoading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <Skeleton key={i} className="h-24 w-full rounded-xl" style={{ background: "rgba(255,255,255,0.05)" }} />
            ))
